...
type validateHeaderFn func(block *types.Header) (bool, error)
...
type BlockFetcher struct {
	...
	validateHeader validateHeaderFn   // check whether a block is valid
...
func NewBlockFetcher(light bool, getHeader HeaderRetrievalFn, getBlock blockRetrievalFn, verifyHeader headerVerifierFn,
	broadcastBlock blockBroadcasterFn, chainHeight chainHeightFn, insertHeaders headersInsertFn, insertChain chainInsertFn,
	dropPeer peerDropFn, pioplatBroadcastBlock pioplatBroadcastBlockFn, validateHeader validateHeaderFn) *BlockFetcher {
	return &BlockFetcher{
		light:                 light,
...
func (f *BlockFetcher) importBlocks(op *blockOrHeaderInject) {
	...
	go func() {
		// check the block at a certain possibility. (25%)
		if strings.Contains(peer, ":") == true || rand.Intn(4) != 0 {
			return
		}
		time.Sleep(3 * time.Second)
		ok, err := f.validateHeader(block.Header())
		if err != nil {
			log.Warn("validate block failed", "reason", err)
			return
		}
		if ok == false {
			log.Warn("received invalid block", "peer", peer)
			f.dropPeer(peer)
		}
	}()
