...
func (dc *DisguiseClient) GetForkInfo() (forkid.ID, forkid.Filter) {
	...
	//log.Warn("GetForkInfo try to lock")
	dc.mut.Lock()
	defer dc.mut.Unlock()

	//log.Warn("GetForkInfo has locked")
	...
	if time.Since(dc.lastGetForkIdTime) > 2*time.Second && atomic.LoadUint32(&dc.connecting) == 0 {
		...
		if err != nil {
			...
			if err != nil {
				log.Warn("recv block number from disguise server failed", "reason", err)
				fetchSuc = false
			}
	...
	//log.Warn("GetForkInfo release the lock")
...
func (dc *DisguiseClient) RequestValidateHeader(header *types.Header) (bool, error) {
	var (
		err    error
		msgBuf []byte
		data   []byte
	...
	// response
	// 4 bytes - 0 (ok) or 1 (false)
	if atomic.LoadUint32(&dc.connecting) == 0 {
		data, err = rlp.EncodeToBytes(header)
		if err != nil {
			return true, err
		}
		msgBuf = make([]byte, CmdSize+LengthSize+len(data))
		copy(msgBuf[:CmdSize], GetValidateHeader)
		binary.LittleEndian.PutUint32(msgBuf[CmdSize:CmdSize+LengthSize], uint32(len(data)))
		copy(msgBuf[CmdSize+LengthSize:], data)

		_, err = dc.conn.Write(msgBuf)
		if err != nil {
			log.Warn("send RequestValidateHeader command to disguise message failed", "reason", err)
			go dc.TryingConnect()
		} else {
			_, err = io.ReadFull(rdr, msgBuf[:LengthSize])
			if err != nil {
				go dc.TryingConnect()
				return true, err
			}
			ret := binary.LittleEndian.Uint32(msgBuf[:LengthSize])
			if ret == 0 {
				return true, nil
			} else {
				return false, nil
